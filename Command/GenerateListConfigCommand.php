 ...
 */
class GenerateListConfigCommand extends ContainerAwareCommand
{
    ...
    protected function configure()
    {
        ...
        $this->setHelp(<<<EOT
The <info>%command.name%</info> generuje pliki konfiguracyjne dla list.
...
EOT
    ...
    protected function addFile($fieldsInfo, $entityName, $path, $output, $associated = false, $rootFolder,$configEntityName)
    {
        ...
        $fileName = $this->directory . DIRECTORY_SEPARATOR . 'ListConfig.php';

        $templating = $this->getContainer()->get('templating');
        $this->isFileNameBusy($fileName);

        $renderedConfig = $templating->render("CorePrototypeBundle:Command:listconfig.template.twig", [
            "namespace" => $entityNamespace,
            "entityName" => $entityName,
            "objectName" => $objectName,
            "lcObjectName" => lcfirst($objectName),
            "fieldsInfo" => $fieldsInfo,
            "listConfigNamespaceName" => $namespace,
            "associated" => $associated,
            "lowerNameSpaceForTranslate" => $lowerNameSpaceForTranslate
        ]);

        file_put_contents($fileName, $renderedConfig);
        $output->writeln(sprintf("List config generated for <info>%s</info>", $entityName));
    }
