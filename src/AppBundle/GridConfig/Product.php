...
class Product extends GridBuilder
{
   ...
   protected function manipulateQuery($grid)
    {
      ...
      $queryBuilderFn = function ($queryBuilder) use($tableAlias) {
       $queryBuilder->select($tableAlias.'.id,'.$tableAlias.'.name,'.'_productCategory.name as productCategory::name');
    ...
    protected function configureColumn($grid)
    {
      ...
      $column = new TextColumn(array('id' => 'productCategory.name', 'title' => 'productCategory.name', 'source' => $grid->getSource(), 'filterable' => true, 'sortable' => true));
      $grid->addColumn($column,$columnOrder=null);
                    
      $grid->setDefaultOrder('id', 'asc');
      $grid->setVisibleColumns(['id','name','productCategory.name']);
      $grid->setColumnsOrder(['id','name','productCategory.name']);
    ...
    */
    ...
    /*
      //$column->setSafe(false); // not convert html entities
      $column = $grid->getColumn('params'); 
      $column->setTitle('Product.params');    
      $column->manipulateRenderCell(function($value, $row) {
       //return strip_tags($value); //use this function when setSafe is false
       return $value;
      });
    ...
    */
    ...
    protected function configureFilter($grid)
    {
          ...
          $grid->setShowFilters(['id','name','productCategory','params']);
